...
class CCE_MASAC(MASAC):
    def __init__(self, 
                 ...
                 device, 
                 offset: int):
        super(CCE_MASAC, self).__init__(state_dim, action_dim, config, device, offset)
    ...
    def sample(self, transition, start_index, agent_index):
        ...
        with torch.no_grad():
                ...
                return s, o, a, r, o_next, s_next, done, magnet/self.mini_batch
    
    def get_target(self, rewards, next_states, next_observations, dones, noise, signal):
        with torch.no_grad():
    ...
    def update(self, transition, buffer_batchsize, noise, agent_index):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            for i in range(int(buffer_batchsize/self.mini_batch)):
                states, observations, actions, rewards, next_observations, next_states, dones, magnet = self.sample(transition, start_index, agent_index)
                ...
                next_q_values_target = self.get_target(rewards, next_states, next_observations, dones, noise, signal)
