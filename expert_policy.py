...
from utils.utils import get_transform_mat, get_auto_pred, add_new_agent, reward_function, seed_everything, save_gap
...
parser.add_argument("--id", type=str, default='0a8dd03b-02cf-4d7b-ae7f-c9e65ad3c900')
...
if args.scenario == 2:
    ...
    new_input_data=add_new_agent(data,0.5, v0_x, v0_y, 1.19, 2665, -2410)
    ...
    new_input_data=add_new_agent(new_input_data,0, 0, 0, 1.57, -8340, -813)
model_state_dict = torch.load('/home/guanren/Multi-agent-competitive-environment/checkpoints/version_4/MASAC_seed1666_scenario1.ckpt')
...
choose_agent.append(torch.nonzero(data['agent']['category']==3,as_tuple=False).item())
...
with torch.no_grad():
    for episode in tqdm(range(config['episodes'])):
        ...
        #     o += transition_list[i]['states'][choose_index]
        #     a += transition_list[i]['actions'][choose_index]
        ...
        # q = agents[choose_index].critic_1(states_critic_input, actions_critic_input)
        # _,_,actions, log_prob = agents[choose_index].actor(observations, scale)
        # v = q - agents[choose_index].log_alpha.exp() * log_prob
