...
def generate_scenario_visualizations(
    ...
    *,
    ...
    all_scenario_files = sorted(Path(argoverse_scenario_dir).rglob("*.parquet"))
    scenario_file_list = (
    ...
    def generate_scenario_visualization(scenario_path: Path) -> None:
        ...
        viz_save_path = Path(viz_output_dir) / f"{scenario_id}.mp4"
...
def traj_pred_graph():
    y = torch.load("./saved_tensors/gt_100.pt")[..., :2].cpu().numpy()
    y_hat = torch.load("./saved_tensors/traj_propose_best_100.pt").cpu().numpy()
    y_best_hat = torch.load("./saved_tensors/traj_refine_best_100.pt").cpu().numpy()
    print(y.shape)
    print(y_best_hat.shape)
    actual_traj_np = y
    predicted_traj_np = y_best_hat
    predicted_traj_propose_np = y_hat
    fig = plt.Figure()

    for i in range(actual_traj_np.shape[0]):
      if i==2:
            
        actual_car_traj = actual_traj_np[i]
            
        # Plot actual trajectory
        plt.plot(actual_car_traj[:, 0], actual_car_traj[:, 1], 'o-', label=f'Actual Car {i+1}')
        #for j in range(predicted_traj_np[i].shape[0]):
           
            
        # Plot predicted trajectory
        predicted_car_traj = predicted_traj_np[i]
                                                
        # Plot mean prediction
        plt.plot(predicted_car_traj[:, 0], predicted_car_traj[:, 1], '.-', label=f'Predicted refined Car {i+1}')
        predicted_car_propose_traj = predicted_traj_propose_np[i]
        plt.plot(predicted_car_propose_traj[:, 0], predicted_car_propose_traj[:, 1], 'b*', label=f'Predicted proposed Car {i+1}')
        break
        # Plot uncertainty ellipse at each time step
        #         for k in range(predicted_car_traj.shape[0]):
        #            x, y, var_x, var_y = predicted_car_traj[k]
        #            ellipse = plt.matplotlib.patches.Ellipse((x, y), np.sqrt(var_x), np.sqrt(var_y), edgecolor='r', facecolor='none')
...
if __name__ == "__main__":
    generate_scenario_visualizations(argoverse_scenario_dir='/home/guanren/QCNet/',
                                         viz_output_dir='/home/guanren/QCNet/figure',
                                         num_scenarios=781,
